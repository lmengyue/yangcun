...
function callBackFun_message()
{
	var subForm=document.getElementById("subForm");
	subForm.submit();
}

//var varLeavewordElement = "lpid=#楼盘编号&type=#楼盘类型&lpname=#楼盘名称&isdp=#是否点评";
function LoadLeavewordForm(varLeavewordElement,objID,w,h)
{
if(!w){
	w=613;
}
...
leaveWord_Initial();
var varLeavewordForm='<form style="padding:0;margin:0" ID="subForm" name="subForm" action="/ask/savemessage.php?'
+varLeavewordElement
+'" method="POST" onsubmit="return false;">'
+'<div class="textname" style=" margin-left:15px;padding:0;"><div style="width:90%;margin:0;padding:0;height:24px;overflow:hidden"><div id="logST" class="textname"></div>'
+'<div class="textname">留言内容：(500字以内)</div><div style="clear:both;width:0;height:0;margin:0;padding:0"></div></div>'
+'<textarea style="width:'+w+'px;height:'+h+'px" onfocus="javascript:document.getElementById(\'showLen1\').innerHTML=this.value.length;" onkeyup="javascript:document.getElementById(\'showLen1\').innerHTML=this.value.length;" name="content" rows="4" id="content" class="txtField"></textarea>'
+'<div class="bt_style"><input id="Submit" type="submit" value="确认，提交" style="cursor:pointer;" onclick="Validate()" /></div>'
+'<div style=" margin-left:15px; height:5px;width:10px;  clear:both;"></div></form>';
document.getElementById(objID).innerHTML = varLeavewordForm;
nEditor = new jtbcEditor('content');
nEditor.tEditUBBMode = 0;
nEditor.tInit('nEditor', '/js/jtbceditor/');
document.getElementById('content-divPanel').style.display='none';
document.getElementById('content-iframe').style.height='100%';
}

function leaveWord_Initial()
{
document.write("<SCRIPT language='JavaScript' src='http://www.myvacation.cn/js/TopLoginStateSet.js' type='text/JavaScript'></SCRIPT> \n");
//document.write("<iframe src=\"http://172.26.12.18/members/domain.htm\" scrolling=\"no\"; width=\"1\" height=\"1\" frameborder=\"0\" allowtransparency=\"true\"></iframe> \n");
}
...
function jtbcEditor(_id)
{
  ...
  this.tLoadToolbar = function(_obj)
  {
    ...
    if (tObj1)
    {
      ...
      for (var tKey1=0;tKey1<tArray1.length;tKey1++)
      {
  ...
  this.tLoadImageTable = function()
  {
    ...
    tLinkTableHTML += '  <tr>';
    tLinkTableHTML += '    <td>' + jtbcEditorLang.tImageURL + '<input id="' + this.tid + '-jtbcEditorImageText" type="text" value="http://" class="jtbcEditorImageText" ondblclick="this.select();" /></td>';
    tLinkTableHTML += '</tr>';
	tLinkTableHTML += '<tr>';
	tLinkTableHTML += '<td><iframe src="/ask/upload.php" height="25" width="100%" scrolling="no" frameborder="0" name="uploadFrame"></iframe></td>';
    tLinkTableHTML += '  </tr>';
    tLinkTableHTML += '  <tr>';
  ...
  this.tInit = function(_name, _baseURL)
  {
    ...
    if (tObj1)
    {
      ...
      var tObj1Width = tObj1.offsetWidth>0?tObj1.offsetWidth:parseInt(tObj1.style.width);
      var tObj1Height = tObj1.offsetHeight>0?tObj1.offsetHeight:parseInt(tObj1.style.height);
